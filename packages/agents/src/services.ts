...
export class LLMService extends Context.Tag("LLMService")<
  LLMService,
  {
    readonly generateText: (prompt: string, model: string) => Effect.Effect<any, Error, string>
    readonly getEmbedding: (text: string) => Effect.Effect<any, Error, number[]>
  }
>() {}
...
export class VectorStoreService extends Context.Tag("VectorStoreService")<
  VectorStoreService,
  {
    readonly storeVector: (id: string, vector: number[]) => Effect.Effect<any, Error, void>
    readonly queryVector: (vector: number[], topK: number) => Effect.Effect<any, Error, string[]>
  }
>() {}
...
export class CMSService extends Context.Tag("CMSService")<
  CMSService,
  {
    ...
    readonly checkArticleExists: (title: string) => Effect.Effect<any, Error, boolean>
  }
>() {}
...
export class DatabaseService extends Context.Tag("DatabaseService")<
  DatabaseService,
  {
    ...
    readonly getProcessStatus: (processId: string) => Effect.Effect<any, Error, ProcessStatus>
    readonly updateProcessStatus: (processId: string, status: ProcessStatus) => Effect.Effect<any, Error, void>
  }
>() {}
...
export const LLMServiceLive = Layer.succeed(
  ...
  LLMService.of({
    generateText: (prompt: string, model: string) =>
      Effect.succeed(`Generated titles based on prompt: ${prompt}`),
...
export const VectorStoreServiceLive = Layer.succeed(
  ...
  VectorStoreService.of({
    ...
    queryVector: (vector: number[], topK: number) => Effect.succeed(["id1", "id2"]),
  })
)
...
export const DatabaseServiceLive = Layer.succeed(
  ...
  DatabaseService.of({
    ...
    updateProcessStatus: (processId: string, status: ProcessStatus) => Effect.succeed(void 0),
  })
)
